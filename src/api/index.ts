...
export const authApi = {
  ...
  signIn: async (credentials: SignInCredentials): Promise<SignInResponse & { user?: User }> => {
    try {
  ...
  resetPassword: async (data: ResetPasswordRequest): Promise<ResetPasswordResponse> => {
    try {
      const response = await axiosInstance.put<ResetPasswordResponse>('/users/password-reset', data)
...
export const profileApi = {
  getCurrentUserProfile: async (): Promise<User> => {
    try {
      ...
      throw { message: 'Failed to fetch user profile', originalError: error }
    }
  },

  createProfile: async (data: Partial<User>): Promise<User> => {
    const response = await axiosInstance.post<UserProfileResponse>('/profile/new', data)
    return response.data.profile
  },
  ...
  getProfilesWithIdeas: async (
    ...
    const response = await axiosInstance.get<ProfileWithIdeasResponse>('/profiles-with-ideas', {
      params: { limit, offset },
    })
    return response.data
  },
  ...
  getUserProfileById: async (userId: string): Promise<UserProfileByIdResponse> => {
    try {
      const response = await axiosInstance.get<UserProfileByIdResponse>(`/profiles/id/${userId}`)
      return response.data
    } catch (error: any) {
      if (error.response?.data) {
        throw error.response.data
...
export const ideaApi = {
  submitIdea: async (data: IdeaSubmissionRequest): Promise<IdeaSubmissionResponse> => {
    try {
      if (!data.user_id) {
        try {
          ...
          console.error('Failed to fetch user profile for idea submission:', profileError)
          throw new Error('User ID is required for idea submission')
        }
      }
      const response = await axiosInstance.post<IdeaSubmissionResponse>('/ideas', data)
      return response.data
    } catch (error: any) {
      if (error.response?.data) {
        throw {
          message: error.response.data.message || 'Failed to submit idea',
